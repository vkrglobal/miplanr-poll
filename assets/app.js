...
function initCreate(){
  ...
  $('pollForm').onsubmit=async ev=>{
    ...
    try{const calendarOptions=isCalendarPoll()?getCalendarOptions():[]; const options=isCalendarPoll()?calendarOptions.map(o=>o.label):[...document.querySelectorAll('.opt')].map(i=>i.value.trim()).filter(Boolean);if(options.length<2){alert('Please add at least two options.');return}
      ...
      const shareTitle=($('title')?.value||'my miPlanr poll').trim();
      const shareQuestion=($('question')?.value||'').trim();
      const suggestedShareMessage=(shareQuestion?`Please vote on ${shareTitle}: ${shareQuestion}
${link}`:`Please vote on ${shareTitle}
${link}`);
      if($('shareMessage')) $('shareMessage').value=suggestedShareMessage;
      $('sharePanel').classList.add('visible');
      $('copyBtn').onclick=()=>navigator.clipboard.writeText(link);
      if($('copyMsgBtn')) $('copyMsgBtn').onclick=()=>navigator.clipboard.writeText(($('shareMessage')?.value||suggestedShareMessage).trim()||link);
      $('waBtn').onclick=()=>window.open('https://wa.me/?text='+encodeURIComponent((($('shareMessage')?.value||suggestedShareMessage).trim())||link),'_blank');
      ...
      $('createMsg').innerHTML='<div class="notice">'+(data.reused_poll?'🔗 Matching poll found. Responses will be consolidated into the existing poll database.':'🎉 Poll created. Share options are now ready.')+'</div><div class="notice"><b>Admin results link:</b><br><input readonly value="'+esc(adminLink)+'" onclick="this.select()"><small>Keep this link for administrators. It can show consolidated results, export to Excel/Google Sheets and sync to roster planning.</small></div>'+inviteHtml;
...
async function initPoll(){
  ...
  function draw(currentTotal=total,currentMap=voteMap){
    const maxForAnyOption=Math.max(0,...Object.values(currentMap||{}).map(Number));
    const quorumReached=isCal?maxForAnyOption>=(p.threshold||3):currentTotal>=(p.threshold||3);
    document.getElementById('voteChoices').innerHTML=enriched.map(o=>{const c=currentMap[o.id]||0;const denom=isCal?Math.max(1,maxForAnyOption,currentTotal):currentTotal;const pct=denom?Math.round(c/denom*100):0;const checked=selected.has(o.id)?'checked':'';const inputType=isCal?'checkbox':'radio';return `<label class="choice vote-option"><input type="${inputType}" name="opt" value="${o.id}" ${checked}><span class="icon-badge">${isCal?esc(o.displayIcon||'📅'):iconHTMLFromResult(o.visual)}</span><div class="choice-content"><b>${esc(o.option_text || o.label || '')}</b><div class="bar" style="width:${pct}%"></div><small>${c} available vote${c===1?'':'s'}${isCal?' for this date':' • '+pct+'%'}</small></div></label>`}).join('')+`<div class="notice">${isCal?'Calendar poll: tick every date you can do. Quorum is counted per date, so one person selecting two dates is one vote on each selected date, not two votes toward the same date.':'One option only.'} ${isCal?maxForAnyOption:currentTotal} vote${(isCal?maxForAnyOption:currentTotal)===1?'':'s'} on the leading option ${quorumReached?'🎉 quorum reached':''}</div>`;
